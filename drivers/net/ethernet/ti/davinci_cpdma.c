...
//    printk(KERN_ERR "cpdma: submit data chan_num: %d, state = %d, dir = %d\n", chan->chan_num, chan->state, chan->dir);

	if (chan->state == CPDMA_STATE_TEARDOWN) {
		ret = -EINVAL;
		printk(KERN_ERR "cpdma: invalid argument\n");
		goto unlock_ret;
	}
	...
	if (!desc) {
		chan->stats.desc_alloc_fail++;
        printk(KERN_ERR "cpdma: no memory\n");
        ret = -ENOMEM;
		goto unlock_ret;
	}
	...
	if (ret) {
		...
		ret = -EINVAL;
        printk(KERN_ERR "cpdma: invalid argument (map single)\n");
		goto unlock_ret;
	}
