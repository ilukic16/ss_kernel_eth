...
#define INT_PIN					28 // we need to change touch interrupt pin - ert
/* Debug levels */
...
static inline void nutouch_report_coordinate(void)
{
	...
	for(i=0; i<NUM_FINGERS_SUPPORTED; i++) {
		...
		if (finger_num > 0) {
			if(finger_num == 1)	
			input_report_key(nutouch->input, BTN_TOUCH, 1);
	...
	/* To re-schedule the Console blank for Screen saver 
         functionality when touch event occured ;  Added 29-07-2015 @ Sanmina*/
...
static const struct of_device_id sps_ts_ids[] = {
	{ .compatible = "NuTouch",},
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, sps_ts_ids);

static int sps_ts_probe(struct i2c_client *client,
		const struct i2c_device_id *id)
{
	...
	error = NUTOUCH_GET_FW_VER(nutouch);
	...
	input_abs_set_res(input, ABS_Y, Y_MAX);
...
static int sps_ts_remove(struct i2c_client *client)
{
	struct nutouch_data *nutouch;
	dev_t dev;
	nutouch = i2c_get_clientdata(client);
	...
	sysfs_remove_group(&nutouch->client->dev.kobj, &nutouch_attribute_group);
	device_destroy(nutouch->dev_classp, MKDEV(nutouch->dev_major, nutouch->dev_minor));
	class_unregister(nutouch->dev_classp);
...
static int ti_sps_suspend(struct i2c_client *client, pm_message_t mesg)
{
	nutouch_debug(DEBUG_ERROR, "NuTouch: nutouch_suspend\n");
	return 0;
}

static int ti_sps_resume(struct i2c_client *client)
{
...
static const struct i2c_device_id sps_ts_id[] = {
	{ "NuTouch", 0},
	{ }
};
MODULE_DEVICE_TABLE(i2c, sps_ts_id);

static struct i2c_driver sps_ts_driver = {
	.driver = {
		.name	= "NuTouch",
		.owner	= THIS_MODULE,
		.of_match_table = sps_ts_ids,
	},
	.probe		= sps_ts_probe,
	.remove		= sps_ts_remove,
	.id_table	= sps_ts_id,
	#ifndef CONFIG_HAS_EARLYSUSPEND
	.suspend    = ti_sps_suspend,
	.resume     = ti_sps_resume,
	#endif
};

module_i2c_driver(sps_ts_driver);

/* Module information */
MODULE_AUTHOR("Sanmina SDC");/* Porting and configuration*/
