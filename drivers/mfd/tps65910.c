...
#define ERT_NMI_IRQ_ONOFF_BUTTON 23

#define FIRST_MINOR 0
#define MINOR_CNT 1

static struct i2c_client *tps65910_i2c_client=NULL;
struct tps65910 *globaltps65910=NULL;


//static irqreturn_t ert_isr_thread(void);
struct dentry *file;
struct siginfo info;
...
static int flag=0;
static int signal_alive_flag=0;
...
static irqreturn_t button_isr_thread()
{	
	static int count;
       ...
       //printk("\n\n\n   Button Pressed......\n\n\n\n");
		...
		ret = tps65910_reg_set_bits(globaltps65910, TPS65910_INT_STS, 0xff);

		if (ret < 0) 
		{
		dev_err(globaltps65910->dev, "clear ck32k_ctrl failed: %d\n", ret);
		return ret;
		}

		ret = tps65910_reg_set_bits(globaltps65910, TPS65910_INT_MSK, 0xff);
...
struct i2c_client local_i2c_client;
...
/******************************************************************/
    ...
    //memcpy(&global_i2c_client,i2c,sizeof(global_i2c_client));

	globaltps65910=devm_kzalloc(&i2c->dev, sizeof(*tps65910), GFP_ATOMIC);

	if (globaltps65910 == NULL)
		return -ENOMEM;

	globaltps65910->of_plat_data = of_pmic_plat_data;
	i2c_set_clientdata(i2c, globaltps65910);
	globaltps65910->dev = &i2c->dev;
	globaltps65910->i2c_client = i2c;
	globaltps65910->id = chip_id;

	globaltps65910->regmap = devm_regmap_init_i2c(i2c, &tps65910_regmap_config);
	if (IS_ERR(globaltps65910->regmap)) {
		ret = PTR_ERR(globaltps65910->regmap);
		dev_err(&i2c->dev, "regmap initialization failed: %d\n", ret);
		return ret;
	}

	//ret = request_threaded_irq(ERT_NMI_IRQ_ONOFF_BUTTON, NULL,button_isr_thread,IRQF_TRIGGER_LOW | IRQF_ONESHOT,
				 // "SPSONOFF", tps65910);

	// ret = request_irq(ERT_NMI_IRQ_ONOFF_BUTTON, button_isr_thread, IRQF_TRIGGER_LOW | IRQF_ONESHOT,
			//	  "SPSONOFF", tps65910);

	
	//irq_set_irq_type(ERT_NMI_IRQ_ONOFF_BUTTON, IRQF_TRIGGER_HIGH );
...
static ssize_t signal_read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
...
static ssize_t signal_write(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	   char data[1];
       unsigned long ret;
	...
	if(data[0] == 2)
	{
			...
			ret = tps65910_reg_clear_bits(globaltps65910, TPS65910_INT_MSK, 0x14);
...
static int __init tps65910_i2c_init(void)
{
    ...
    if ((ret = alloc_chrdev_region(&dev, FIRST_MINOR, MINOR_CNT, "pmic_ioctl")) < 0)
    {
        return ret;
    }
    ...
    if (IS_ERR(dev_ret = device_create(cl, NULL, dev, NULL, "powerbutton")))
    {
        class_destroy(cl);
        cdev_del(&c_dev);
        unregister_chrdev_region(dev, MINOR_CNT);
	...
	printk(" SPS-Power- Module Inserted \n");
