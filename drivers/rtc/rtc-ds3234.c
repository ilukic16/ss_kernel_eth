...
static int ds3234_clear_oscillator_stop_flag(struct device *dev)
{
	/* clear the OscillatorStopFlag (OSF), bit 7 */
	unsigned char tmp;

	ds3234_get_reg(dev, DS3234_REG_CONT_STAT, &tmp);
	ds3234_set_reg(dev, DS3234_REG_CONT_STAT, tmp & 0x7f);
...
static int ds3234_set_time(struct device *dev, struct rtc_time *dt)
{
	...
	ds3234_clear_oscillator_stop_flag(dev);
...
static int ds3234_probe(struct spi_device *spi)
{
	...
	ds3234_set_reg(&spi->dev, DS3234_REG_CONT_STAT, tmp & 0x80);	// Keep the OSF-Flag
