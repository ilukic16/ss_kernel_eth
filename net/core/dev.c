...
static unsigned char filter_addr[][6] =
        {
            {0x68, 0xC9, 0x0B, 0xB6, 0x32, 0xA9},   // dev2
            {0x68, 0xC9, 0x0B, 0xBF, 0x1E, 0xCE},   // dev1
            {0x00, 0x0C, 0x29, 0xDD, 0xE8, 0x39},   // ubuntu
            {0x78, 0x2B, 0xCB, 0x94, 0xDE, 0xA5},   // pc
            {0x00, 0x50, 0x56, 0xC0, 0x00, 0x01},   // vm1
            {0x00, 0x50, 0x56, 0xC0, 0x00, 0x08},   // vm2
        };
...
static int __netif_receive_skb_core(struct sk_buff *skb, bool pfmemalloc)
{
    ...
    unsigned char * src = skb_mac_header(skb);
...
//    int print_src = 1;
//    int print_dst = 1;
//    for (i = 0; i < ((sizeof filter_addr) / 6);++i)
//    {
//        print_src = 1;
//        print_dst = 1;
//        for (j = 0;((print_src == 1 || print_dst == 1) && (j < 6));++j)
//        {
//            if ((print_src == 1) && (src[j] != filter_addr[i][j]))
//            {
//                print_src = 0;
//            }
//
//            if ((print_dst == 1) && (dst[j] != filter_addr[i][j]))
//            {
//                print_dst = 0;
//            }
//
...
//
//        // print?
//        if (print_src || print_dst)
//        {
//            printk(KERN_ERR "src MAC=%02X:%02X:%02X:%02X:%02X:%02X\n", src[0], src[1], src[2], src[3], src[4], src[5]);
//            printk(KERN_ERR "dst MAC=%02X:%02X:%02X:%02X:%02X:%02X\n", dst[0], dst[1], dst[2], dst[3], dst[4], dst[5]);
//            printk(KERN_ERR "P_TYPE = 0x%02X%02X\n", *(proto + 1), *(proto)); // big endian?
//
//            break;
//        }
//    }
...
//    printk(KERN_ERR "++ net/core/dev: netif receive core! skb->dev = %d; skb->sk = %d\n", skb->dev, skb->sk);
...
//    printk(KERN_ERR "++ net/core/dev: ok 1!\n");
...
//    printk(KERN_ERR "++ net/core/dev: ok 2!\n");
...
//    printk(KERN_ERR "++ net/core/dev: ok 3!\n");
...
//    printk(KERN_ERR "++ net/core/dev: ok 4!\n");
...
//    printk(KERN_ERR "++ net/core/dev: ok 5!\n");
...
{
    printk(KERN_ERR "++ net/core/dev: napi gro received package!\n");
